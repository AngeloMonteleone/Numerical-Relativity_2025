...
# different values of the courant factor. The output is a plot of the evolution of the norms in which each line represents 
# the evolution of the norm for a different value of the courant factor
import numpy as np
...
for c_f in [0.3,0.5,0.8]:#VALUES OF THE COURANT FACTOR TO USE (Note, this parts need to be changed if the chosen scheme is FTCS)
    ...
    while(frame<=frames_number):
        if(method == "LEAPFROG"):
            ...
            if(frame == 0):
                ...
                norms.append(np.sqrt(np.sum(u_curr**2)/J))
            else:
                u_new = advance_Leapfrog(u_curr,u_old)
                u_old = u_curr.copy()
                u_curr = u_new.copy()
                norms.append(np.sqrt(np.sum(u_new**2)/J))
        else:
